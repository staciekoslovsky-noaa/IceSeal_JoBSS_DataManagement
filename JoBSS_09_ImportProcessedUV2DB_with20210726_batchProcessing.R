...
# RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_jobss.tbl_detections_processed_uv")
...
for (i in 1:nrow(folders)) {
  ...
  processed_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_jobss.tbl_detections_processed_uv")
  processed_id$max <- ifelse(is.na(processed_id$max), 0, processed_id$max)
  
  files <- list.files(folders$folder_path[i])
  uv_validated <- files[grepl('_uv_irDetectionsTransposed_processed', files)] 
  if(length(uv_validated) == 0) next
  
  processed <- read.csv(paste(folders$folder_path[i], uv_validated, sep = "\\"), skip = 2, header = FALSE, stringsAsFactors = FALSE, col.names = c("detection", "image_name", "frame_number", "bound_left", "bound_top", "bound_right", "bound_bottom", "score", "length", "detection_type", "type_score", 
  ...
  processed <- data.frame(lapply(processed, function(x) {gsub("\\(trk-atr\\) *", "", x)})) %>%
    ...
    mutate(detection_file = uv_validated) %>%
    ...
    select("id", "detection", "image_name", "frame_number", "bound_left", "bound_top", "bound_right", "bound_bottom", "flight", "camera_view", "detection_id", "detection_file")
  
  # Import data to DB
  RPostgreSQL::dbWriteTable(con, c("surv_jobss", "tbl_detections_processed_uv"), processed, append = TRUE, row.names = FALSE)
}
