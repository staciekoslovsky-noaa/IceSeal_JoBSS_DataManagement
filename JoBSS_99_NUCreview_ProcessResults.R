...
RPostgreSQL::dbSendQuery(con, paste("UPDATE ", update_schema, ".", update_table, " SET ", update_field, " = \'Y\' WHERE (image_name) in (SELECT image_name FROM ", update_schema, ".temp_nonnuc)", sep = ""))
RPostgreSQL::dbSendQuery(con, paste("UPDATE ", update_schema, ".", update_table, " SET ", update_field, " = \'N\' WHERE ", update_field, " <> \'Y\' AND (image_name) in (SELECT image_name FROM ", update_schema, ".temp_images)", sep = ""))
